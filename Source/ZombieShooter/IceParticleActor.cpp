...
// Sets default values
AIceParticleActor::AIceParticleActor()
{
	...
	SphereCollision->OnComponentBeginOverlap.AddDynamic(this, &AIceParticleActor::OnOverlapBegin);
...
void AIceParticleActor::BeginPlay()
{
	Super::BeginPlay();
	
	//die after 3 seconds
	FTimerHandle timer;
	GetWorldTimerManager().SetTimer(timer, this, &AIceParticleActor::Die, 3.0f);
}

// Called every frame
void AIceParticleActor::Tick(float DeltaTime)
{
...
void AIceParticleActor::Die()
{
	if (Zombie)
	{
		Zombie->SetIsIced(false);
	}
	Destroy();
}

void AIceParticleActor::OnOverlapBegin(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
